...
static void gdo_event_handler(const gdo_status_t *status, gdo_cb_event_t event, void *arg) {
  ...
  switch (event) {
    ...
    case GDO_CB_EVENT_DOOR_POSITION: {
      ...
      ESP_LOGI(TAG, "Door: %s, %.0f%%, target: %.0f%%", gdo_door_state_to_string(status->door), position * 100,
               target * 100);
...
void GDOComponent::dump_config() { ESP_LOGCONFIG(TAG, "Setting up secplus GDO ..."); }
